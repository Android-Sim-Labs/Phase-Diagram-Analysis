function P = read_P_run_files(varargin)
...
for i = 1:nf
    ...
    while ~feof(fid)
        fline = fgetl(fid);
        k = regexp(fline,'^let','start');
%         k = strfind(fline,'let');
        if isempty(k), continue, end
        [pnames,values] = strtok(fline(k+3:end),'=');
        m = regexp(pnames,'(\w*(?=,|\s))|\w*','match');
%         m = regexp(pnames,'\w*(?=,|\s)','match');
        v = regexp(values(2:end),'[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?','match');
%         v = regexp(values(2:end),'(\d*\.\d*)|\d*','match');
        v = str2num(char(v));
%         v = sscanf(values(2:end),'%f,',Inf); % strread?
        nm = length(m);
        nv = length(v);
        if nm ~= nv
