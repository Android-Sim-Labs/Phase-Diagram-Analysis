...
%             [x2,resnorm,residual] = lsqlin(B,S(:,s),A,b,[],[],[],[],x0(:,s),options);
            [x,resnorm,residual] = lsqlin(B,S(:,s),[],[],Aeq,beq(s),[],[],x0(:,s),options);
...
%         x0 = [1 D_con(1,1) D_con(end,1) C(1)+0.0001 C(end)-0.0001];
        x0 = [1 maxa mina C(1) C(end)];
        lb = [0.1 mina mina C(1) C(1)];
        ub = [10 maxa maxa C(end) C(end)];
%         x0 = [1 C(1) C(end)];
%         lb = [0.1 C(1) C(1)];
%         ub = [10 C(end) C(end)];
%         [x,resnorm,exitflag] = fminsearchcon(@phase_boundary_fit_fun,x0,lb,ub,[],[],[],options,C,basis_method,D_con(:,1));
